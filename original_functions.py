...
def get_goodvibes_e(dataframe, temp): #uses goodvibes to calculate thermochemical energy values, requires frequency job
    ...
    for index, row in dataframe.iterrows():
        try:
            ...
            general_file_info = io.read_initial(str(log_file) + ".log")

            # Grab level of theory, solvation model, check for Normal Termination
            # These variable names are taken from GoodVibes source code
            l_o_t = general_file_info[0]
            s_m = general_file_info[1]
            progress_of_file = general_file_info[2]
            orientation_of_file = general_file_info[3]
            grid_of_file = general_file_info[4]

            # Carry out the thermochemical analysis - auto-detect the vibrational scaling factor
            # These values were taken from the original GV_Options() class
            bbe_val = thermo.calc_bbe(file=str(log_file) + ".log",
                                      spc='link',
                                      temperature=temp,
                                      freq_scale_factor=1,
                                      QS='grimme',
                                      ...
                                      s_freq_cutoff=100.0,
                                      H_FREQ_CUTOFF=100.0,
                                      conc= gv.ATMOS/ (thermo.GAS_CONSTANT * temp),
                                      solv=s_m,
                                      invert=False,
                                      d3_term=0
                                      )
